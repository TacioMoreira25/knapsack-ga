...
public class Charts {
    ...
    public static void gerarGraficoEvolucaoFitness(GA ga) {
        ...
        JFreeChart chart = ChartFactory.createXYLineChart(
            String.format(
                "Evolução do Fitness (%d pop, %d gen)",
    ...
    public static void gerarGraficoTempoCapacidade(
        List<Experiment.ExperimentResult> dados
    ) {
        System.out.println(
            "  Gerando gráfico de Tempo (Médio) vs Capacidade..."
        );
    ...
    public static void gerarGraficoFitnessCapacidade(
        List<Experiment.ExperimentResult> dados
    ) {
        System.out.println(
            " Gerando gráfico de Fitness (Médio) vs Capacidade..."
        );
    ...
    public static void gerarGraficoTempoItens(
        List<Experiment.ExperimentResult> dados
    ) {
    ...
    public static void gerarGraficoFitnessItens(
        List<Experiment.ExperimentResult> dados
    ) {
        System.out.println(
            " Gerando gráfico de Fitness (Médio) vs Conjuntos..."
        );
    ...
    public static void gerarGraficoDiversidade(GA ga) {
        ...
        double[] historicoDiversidade = ga.getDiversityHistory();

        int lastGen = ga.getConvergenceGeneration() > 0
            ? ga.getConvergenceGeneration()
            : historicoDiversidade.length;
    ...
    public static void gerarGraficoTaxaSucesso(
        List<Experiment.ExperimentResult> capDados,
        ...
        for (int i = 0; i < itensDados.size() && i < itemLabels.length; i++) {
            dataset.addValue(
                itensDados.get(i).averageFitness,
                "Sucesso (%)",
                "Itens " + itemLabels[i]
            );
        }
    ...
    public static void gerarTodosGraficos(
        GA ga,
        List<Experiment.ExperimentResult> capDados,
        List<Experiment.ExperimentResult> itensDados
    ) {
